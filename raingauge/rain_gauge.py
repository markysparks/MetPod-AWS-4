...
from daily_rainfall import DAILY_RAINFALL
...
class RainGaugeSetup:
    ...
    def __init__(self, client, mqtt_topic, mqtt_qos):
        ...
        # The Raspberry Pi GPIO pin to which one wire of the tipping
        ...
        self.rain_data = OrderedDict(
            [('rainrate', 0.0), ('rainaccum', 0.0), ('units', self.units)])
    ...
    def tx_message(self):
        ...
        if self.units.upper() == 'INCH/HR':
            ...
            self.rain_data.update(
                dict(rainrate=self.bucket_tip_handler.rate,
                     rainaccum=self.daily_rainfall.get_total(),
                     units=self.units))
    ...
    def rain_tip_event(self, channel):
       ...
       GPIO pin status is made to avoid false/noise contacts"""
        ...
        if GPIO.input(channel) == 0:
            ...
            # Get the latest rainfall amount, adding the tip amount that
            # has just occurred.
            self.rain_data.update(dict(
                rainaccum=self.daily_rainfall.get_total(self.amount_per_tip)))
